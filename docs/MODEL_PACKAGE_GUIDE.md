...
## Optional: multi-model composition (advanced)

If you need multiple submodels (e.g., hospitalization → chronic disease) on a single canonical patient time axis, see:

- `docs/MULTI_MODEL_COMPOSITION.md`
- `R/08_multi_model_composition.R`
- `docs/examples/08_multi_model_toy.R`

This extension introduces *namespaced state*, per-model scope (`core$model_active`), and explicit handoff payloads. The standard single-model workflow in Scripts 01–07 is unchanged.
