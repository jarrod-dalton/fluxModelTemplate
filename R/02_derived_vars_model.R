...
# In fluxCore, derived variables are stored on the Entity object as a
# named list of functions:
#   f(entity, j, t) -> scalar (or NULL)
#
# Notes
#   - Derived vars should depend on core state and/or event history.
#   - Keep derived vars non-recursive (do not call entity$snapshot() inside).
#   - If you need history, use entity$events (event log) and entity$state_at(...)
...
derived_vars_model <- function(params = list()) {
  ...
  bp_controlled <- function(entity, j, t) {
  ...
  n_no_show <- function(entity, j, t) {
    # entity$events is a data.frame-like event log.
    ev <- entity$events
